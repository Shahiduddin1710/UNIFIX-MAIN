...
const CACHE_KEY_FEED = "lostfound_feed";
const CACHE_KEY_MYPOSTS = "lostfound_myposts";
const CACHE_KEY_CLAIMS = "lostfound_claims";

let _cachedFeed: LostItem[] = [];
let _cachedMyPosts: LostItem[] = [];
let _cachedClaims: ClaimItem[] = [];
...
export default memo(function StaffFoundScreen() {
 ...
 const [feedItems, setFeedItems] = useState<LostItem[]>(_cachedFeed);
  const [myPosts, setMyPosts] = useState<LostItem[]>(_cachedMyPosts);
  const [claimItems, setClaimItems] = useState<ClaimItem[]>(_cachedClaims);
...
const fetchLostFound = useCallback(async (uid: string, silent = false) => {
    try {
      if (!silent) {
        const [cachedFeed, cachedMyPosts, cachedClaims] = await Promise.all([
          loadCache(CACHE_KEY_FEED),
          loadCache(CACHE_KEY_MYPOSTS + uid),
          loadCache(CACHE_KEY_CLAIMS),
        ]);
        if (cachedFeed) setFeedItems(cachedFeed);
        if (cachedMyPosts) setMyPosts(cachedMyPosts);
        if (cachedClaims) setClaimItems(cachedClaims);
        if (cachedFeed || cachedMyPosts || cachedClaims) setLoading(false);
      }

      const [feedSnap, myPostsSnap, claimsSnap] = await Promise.all([
        getDocs(query(collection(db, "lostFound"), where("status", "==", "available"), orderBy("createdAt", "desc"))),
        getDocs(query(collection(db, "lostFound"), where("postedBy", "==", uid), orderBy("createdAt", "desc"))),
        getDocs(query(collection(db, "claims"), orderBy("createdAt", "desc"))),
      ]);

      const feed = feedSnap.docs.map((d) => ({ id: d.id, ...(d.data() as any), isMyPost: d.data().postedBy === uid }));
      const myPosts = myPostsSnap.docs.map((d) => ({ id: d.id, ...(d.data() as any), isMyPost: true }));
      const claims = claimsSnap.docs.map((d) => ({ id: d.id, ...(d.data() as any) }));

_cachedFeed = feed;
      _cachedMyPosts = myPosts;
      ...
      await Promise.all([
        saveCache(CACHE_KEY_FEED, feed),
        saveCache(CACHE_KEY_MYPOSTS + uid, myPosts),
        saveCache(CACHE_KEY_CLAIMS, claims),
      ]);
    } catch {}
  }, []);
  ...
  }, [handedToName, handoverItem, staffUid, fetchLostFound]);

  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    if (staffUid) await fetchLostFound(staffUid);
    setRefreshing(false);
  }, [staffUid, fetchLostFound]);
...
const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      const silent = isLoaded("staffFound");
      await fetchLostFound(u.uid, silent);
