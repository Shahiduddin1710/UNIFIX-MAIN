import {
  ...
  ScrollView, RefreshControl, Modal, Image, TextInput,
...
type Complaint = {
  ...
  status: string; submittedByName: string; submittedByEmail: string;
  submittedByRole: string; submittedByPhone?: string;
  photoUrl?: string | null; createdAt: any; assignedTo?: string;
  assignedToName?: string;
  rejectedBy?: { uid: string; name: string; reason: string; rejectedAt: string }[];
  assignableTo?: string[];
};
...
type StaffData = {
  fullName: string; email: string; designation: string;
  employeeId: string; experience: string; phone?: string;
  photoUrl?: string; gender?: string; nationalIdCardUrl?: string;
};
...
const handleHandover = async () => {
  ...
  const handleSaveProfile = async () => {
    ...
    if (editPhone && !/^[6-9]\d{9}$/.test(editPhone.trim())) { setProfileError("Enter a valid 10-digit phone number."); return; }
    ...
    try {
      ...
      if (u) {
        ...
        setStaffData(prev => prev ? { ...prev, fullName: editName.trim(), phone: editPhone.trim() } : prev);
      }
  ...
  const handleLogoutAllDevices = () => {
    Alert.alert("Logout All Devices", "This will end all active sessions.", [
      ...
      { text: "Confirm", style: "destructive", onPress: async () => {
        try { await authAPI.logoutAllDevices(); await auth.signOut(); router.replace("/login" as any); } catch {}
      }},
    ]);
  };

  const handleDeleteAccount = () => {
    Alert.alert("Delete Account", "Your deletion request will be sent to admin for approval.", [
      { text: "Cancel", style: "cancel" },
      { text: "Submit Request", style: "destructive", onPress: async () => {
        try {
          const data = await authAPI.deleteAccount();
          if (data.requiresApproval) Alert.alert("Request Submitted", "Your account deletion request has been submitted and is currently under review.");
        } catch (err: any) { Alert.alert("Error", err.message || "Failed to submit request."); }
      }},
    ]);
  };
  ...
  const uid = staffUid ?? "";
  const pending   = allComplaints.filter(c => c.status === "pending" && Array.isArray(c.assignableTo) && c.assignableTo.includes(uid));
  const active    = allComplaints.filter(c => (c.status === "assigned" || c.status === "in_progress") && c.assignedTo === uid);
  const completed = allComplaints.filter(c => c.status === "completed" && c.assignedTo === uid);
  const rejected  = allComplaints.filter(c => Array.isArray(c.rejectedBy) && c.rejectedBy.some(r => r.uid === uid));
  ...
  const STAT_ITEMS = [
    { iconName: "clipboard-outline" as keyof typeof Ionicons.glyphMap, iconBg: "#f0fdf4", iconColor: "#16a34a", label: "PENDING",     value: pending.length,                                          sub: `+${Math.min(pending.length, 3)} since morning`, subColor: "#8b5cf6" },
    { iconName: "flash-outline"     as keyof typeof Ionicons.glyphMap, iconBg: "#fef3c7", iconColor: "#d97706", label: "IN PROGRESS", value: active.filter(c => c.status === "in_progress").length,   sub: `${Math.min(active.length, 4)} Priority`,        subColor: "#d97706" },
    { iconName: "checkmark-circle-outline" as keyof typeof Ionicons.glyphMap, iconBg: "#dcfce7", iconColor: "#16a34a", label: "COMPLETED",  value: completed.length,                                sub: "Last 7 days",                                   subColor: "#16a34a" },
    { iconName: "star-outline"      as keyof typeof Ionicons.glyphMap, iconBg: "#fef3c7", iconColor: "#f59e0b", label: "RATING",      value: avgRating !== null ? `${avgRating}/5` : "—",             sub: ratingCount > 0 ? `${ratingCount} rating${ratingCount > 1 ? "s" : ""}` : "No ratings yet", subColor: "#f59e0b" },
  ];
  ...
  const renderTaskCard = (c: Complaint) => {
    ...
    return (
      <View key={c.id} style={s.taskCard}>
        {c.photoUrl ? (
          <TouchableOpacity onPress={() => setImageViewerUri(c.photoUrl!)} activeOpacity={0.9} style={{ position: "relative" }}>
            <Image source={{ uri: c.photoUrl }} style={s.taskPhoto} resizeMode="cover" />
            <View style={[s.urgentBadge, { backgroundColor: sc.color }]}><Text style={s.urgentBadgeText}>{sc.label.toUpperCase()}</Text></View>
          </TouchableOpacity>
        ) : null}
        <View style={s.taskBody}>
          {!c.photoUrl && (
            <View style={[s.statusBadgeTop, { backgroundColor: sc.bg }]}>
              <Text style={[s.statusBadgeTopText, { color: sc.color }]}>{sc.label}</Text>
            </View>
          )}
          ...
          <View style={s.taskLocationRow}>
            ...
            <Text style={s.taskLocationText} numberOfLines={1}>{location}</Text>
          </View>
          <View style={s.taskReporterRow}>
            <View style={s.taskReporterAvatar}><Text style={s.taskReporterAvatarText}>{c.submittedByName?.[0]?.toUpperCase()}</Text></View>
            <Text style={s.taskReporterName}>Reporter: {c.submittedByName}</Text>
          </View>
          {c.status === "completed" && (c as any).rating != null && (
            <View style={s.taskRatingRow}>
              {[1,2,3,4,5].map(star => (
                <Ionicons key={star} name={star <= (c as any).rating ? "star" : "star-outline"} size={14} color={star <= (c as any).rating ? "#f59e0b" : "#e2e8f0"} />
              ))}
              <Text style={s.taskRatingText}>{(c as any).rating}/5 — rated by reporter</Text>
            </View>
          )}
          <View style={s.taskBtnRow}>
            {c.status === "pending" && (
              <>
                <TouchableOpacity style={s.acceptBtn} onPress={() => handleAccept(c.id)} disabled={actionLoading === c.id}>
                  {actionLoading === c.id ? <ActivityIndicator color="#fff" size="small" /> : <Text style={s.acceptBtnText}>Accept</Text>}
                </TouchableOpacity>
                <TouchableOpacity style={s.detailBtn} onPress={() => { setSelectedComplaint(c); setDetailVisible(true); }}><Text style={s.detailBtnText}>View Details</Text></TouchableOpacity>
              </>
            )}
            {c.status === "assigned" && (
              <>
                <TouchableOpacity style={s.progressBtn} onPress={() => handleUpdateStatus(c.id, "in_progress")} disabled={actionLoading === c.id}>
                  {actionLoading === c.id ? <ActivityIndicator color="#fff" size="small" /> : <Text style={s.acceptBtnText}>Start Work</Text>}
                </TouchableOpacity>
                <TouchableOpacity style={s.detailBtn} onPress={() => { setSelectedComplaint(c); setDetailVisible(true); }}><Text style={s.detailBtnText}>View Details</Text></TouchableOpacity>
              </>
            )}
            {c.status === "in_progress" && (
              <>
                <TouchableOpacity style={s.acceptBtn} onPress={() => handleUpdateStatus(c.id, "completed")} disabled={actionLoading === c.id}>
                  {actionLoading === c.id ? <ActivityIndicator color="#fff" size="small" /> : <Text style={s.acceptBtnText}>Mark Complete</Text>}
                </TouchableOpacity>
                <TouchableOpacity style={s.detailBtn} onPress={() => { setSelectedComplaint(c); setDetailVisible(true); }}><Text style={s.detailBtnText}>View Details</Text></TouchableOpacity>
              </>
            )}
            {(c.status === "completed" || c.status === "rejected") && (
              <TouchableOpacity style={[s.detailBtn, { flex: 1 }]} onPress={() => { setSelectedComplaint(c); setDetailVisible(true); }}><Text style={s.detailBtnText}>View Details</Text></TouchableOpacity>
            )}
          </View>
        </View>
      </View>
  ...
  const renderRejectedCard = (c: Complaint) => {
    ...
    const catLabel = c.category.charAt(0).toUpperCase() + c.category.slice(1);
    const title = c.subIssue || c.customIssue || "Issue";
    const myRejection = Array.isArray(c.rejectedBy) ? c.rejectedBy.find(r => r.uid === uid) : null;
    return (
      <View key={c.id} style={s.taskCard}>
        {c.photoUrl ? (
          <TouchableOpacity onPress={() => setImageViewerUri(c.photoUrl!)} activeOpacity={0.9}>
            <Image source={{ uri: c.photoUrl }} style={s.taskPhoto} resizeMode="cover" />
            <View style={[s.urgentBadge, { backgroundColor: "#dc2626" }]}><Text style={s.urgentBadgeText}>REJECTED</Text></View>
          </TouchableOpacity>
        ) : null}
        <View style={s.taskBody}>
          {!c.photoUrl && <View style={[s.statusBadgeTop, { backgroundColor: "#fef2f2" }]}><Text style={[s.statusBadgeTopText, { color: "#dc2626" }]}>Rejected by You</Text></View>}
          <Text style={s.taskTitle} numberOfLines={2}>{title}</Text>
          <View style={s.taskCatBadge}>
            <Ionicons name={catIconName} size={12} color="#16a34a" />
            <Text style={s.taskCatText}>{catLabel.toUpperCase()}</Text>
          ...
          <View style={s.taskLocationRow}>
            <Ionicons name="location-outline" size={13} color="#64748b" />
            <Text style={s.taskLocationText} numberOfLines={1}>{c.building || "Campus"}</Text>
          </View>
          <View style={s.taskReporterRow}>
            <View style={s.taskReporterAvatar}><Text style={s.taskReporterAvatarText}>{c.submittedByName?.[0]?.toUpperCase()}</Text></View>
            <Text style={s.taskReporterName}>Reporter: {c.submittedByName}</Text>
          ...
          {myRejection?.reason ? (
            <View style={s.rejectedReasonBox}><Text style={s.rejectedReasonLabel}>Your reason:</Text><Text style={s.rejectedReasonText}>{myRejection.reason}</Text></View>
          ) : null}
          <TouchableOpacity style={[s.detailBtn, { marginTop: 10 }]} onPress={() => { setSelectedComplaint(c); setDetailVisible(true); }}><Text style={s.detailBtnText}>View Details</Text></TouchableOpacity>
        </View>
      </View>
    );
  };
    ...
    return (
      <View key={item.id} style={s.lfCard}>
        ...
        <View style={s.lfBody}>
          ...
          <View style={s.lfMetaRow}><Ionicons name="location-outline" size={13} color="#374151" /><Text style={s.lfLocationText}>Room {item.roomNumber}{item.roomLabel ? ` — ${item.roomLabel}` : ""}</Text></View>
          ...
          {isHandedOver ? (
            ...
            <TouchableOpacity style={s.lfHandoverBtn} onPress={() => { setHandoverItem(item); setHandedToName(""); setHandoverError(""); }}>
  ...
  const renderProfileMain = () => (
    <ScrollView style={s.tabScroll} contentContainerStyle={[s.tabContainer, { paddingBottom: bottomNavHeight + 20 }]} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#16a34a"]} />}>
      <View style={s.subHeader}><Text style={s.subHeaderTitle}>My Profile</Text></View>
      <View style={s.profileCard}>
        <TouchableOpacity onPress={handlePickPhoto} activeOpacity={0.85} style={s.profileAvatarBtn}>
          {staffData?.photoUrl
            ...
            : <View style={s.profileAvatar}><Text style={s.profileAvatarText}>{staffData?.fullName?.[0]?.toUpperCase() || "S"}</Text></View>}
          <View style={s.profileCameraBtn}>
            {photoUploading ? <ActivityIndicator size="small" color="#fff" /> : <Ionicons name="camera" size={13} color="#fff" />}
          </View>
        </TouchableOpacity>
        <Text style={s.profileName}>{staffData?.fullName || "Staff Member"}</Text>
        <View style={s.profileRoleBadge}><Text style={s.profileRoleBadgeText}>MAINTENANCE STAFF</Text></View>
        <Text style={s.profileHint}>{photoUploading ? "Uploading..." : "Tap photo to change"}</Text>
        <View style={s.idBadge}><Text style={s.idBadgeText}>Staff ID: {staffData?.employeeId || "—"}</Text></View>
        <View style={s.profilePerfBox}>
          ...
          <Text style={s.profilePerfDesc}>Total: {completed.length} tasks completed this month</Text>
          <View style={s.progressBarWrap}><View style={[s.progressBarFill, { width: `${Math.min(completed.length * 2, 100)}%` as any }]} /></View>
        </View>
      </View>
      <View style={s.settingsList}>
        {[
          { icon: "person-outline" as keyof typeof Ionicons.glyphMap, bg: "#f8fafc", label: "Personal Information", color: "#0f172a", onPress: () => { setEditName(staffData?.fullName || ""); setEditPhone(staffData?.phone || ""); setProfileError(""); setProfileSuccess(""); setProfileScreen("personalInfo"); } },
          { icon: "notifications-outline" as keyof typeof Ionicons.glyphMap, bg: "#f8fafc", label: "Notifications", color: "#0f172a", onPress: () => {} },
          { icon: "lock-closed-outline" as keyof typeof Ionicons.glyphMap, bg: "#f0fdf4", label: "Change Password", color: "#0f172a", onPress: () => { setCurrentPassword(""); setNewPassword(""); setConfirmPassword(""); setPwError(""); setPwSuccess(""); setProfileScreen("changePassword"); } },
          { icon: "phone-portrait-outline" as keyof typeof Ionicons.glyphMap, bg: "#f0fdf4", label: "Logout from all devices", color: "#0f172a", onPress: handleLogoutAllDevices },
          { icon: "trash-outline" as keyof typeof Ionicons.glyphMap, bg: "#fef2f2", label: "Delete Account", color: "#dc2626", onPress: handleDeleteAccount },
          { icon: "shield-outline" as keyof typeof Ionicons.glyphMap, bg: "#fff7ed", label: "Report Security Issue", color: "#0f172a", onPress: () => { setSecurityIssueType(""); setSecurityDescription(""); setSecurityError(""); setSecuritySuccess(""); setProfileScreen("reportSecurity"); } },
          { icon: "log-out-outline" as keyof typeof Ionicons.glyphMap, bg: "#fef2f2", label: "Log Out", color: "#dc2626", onPress: async () => { await auth.signOut(); router.replace("/login" as any); } },
        ].map((item, index, arr) => (
          <TouchableOpacity key={item.label} style={[s.settingRow, index === arr.length - 1 && { borderBottomWidth: 0 }]} onPress={item.onPress}>
            <View style={[s.settingIcon, { backgroundColor: item.bg }]}>
              <Ionicons name={item.icon} size={18} color={item.color === "#dc2626" ? "#dc2626" : "#374151"} />
            </View>
  ...
  const renderPersonalInfo = () => (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView style={s.tabScroll} contentContainerStyle={[s.tabContainer, { paddingBottom: 40 }]} showsVerticalScrollIndicator={false}>
        <View style={s.subPageHeader}>
          <TouchableOpacity style={s.backBtn} onPress={() => setProfileScreen("main")}><Ionicons name="arrow-back" size={18} color="#0f172a" /></TouchableOpacity>
          <Text style={s.subPageTitle}>Personal Information</Text>
        </View>
        <View style={s.formCard}>
          <Text style={s.formSectionLabel}>BASIC INFO</Text>
          <View style={s.formField}><Text style={s.formLabel}>Full Name</Text><TextInput style={s.formInput} value={editName} onChangeText={setEditName} placeholder="Full name" placeholderTextColor="#9ca3af" autoCapitalize="words" /></View>
          <View style={s.formField}><Text style={s.formLabel}>Phone Number</Text><TextInput style={s.formInput} value={editPhone} onChangeText={setEditPhone} placeholder="10-digit phone" placeholderTextColor="#9ca3af" keyboardType="phone-pad" maxLength={10} /></View>
        ...
        {staffData?.nationalIdCardUrl && (
          <View style={s.formCard}>
            ...
            <Image source={{ uri: staffData.nationalIdCardUrl }} style={s.idCardPreview} resizeMode="contain" />
            <View style={s.privacyNote}><Ionicons name="lock-closed-outline" size={16} color="#64748b" /><Text style={s.privacyNoteText}>Your ID card is only visible to you and Admin.</Text></View>
          </View>
        )}
        <View style={s.formCard}>
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView style={s.tabScroll} contentContainerStyle={[s.tabContainer, { paddingBottom: 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.formCard}>
          ...
          <View style={s.formField}><Text style={s.formLabel}>Current Password</Text><View style={s.pwInputWrap}><TextInput style={s.pwInput} value={currentPassword} onChangeText={setCurrentPassword} placeholder="Current password" placeholderTextColor="#9ca3af" secureTextEntry={!showCurrentPw} /><TouchableOpacity onPress={() => setShowCurrentPw(!showCurrentPw)}><Ionicons name={showCurrentPw ? "eye-off-outline" : "eye-outline"} size={18} color="#94a3b8" /></TouchableOpacity></View></View>
          <View style={s.formField}><Text style={s.formLabel}>New Password</Text><View style={s.pwInputWrap}><TextInput style={s.pwInput} value={newPassword} onChangeText={setNewPassword} placeholder="New password" placeholderTextColor="#9ca3af" secureTextEntry={!showNewPw} /><TouchableOpacity onPress={() => setShowNewPw(!showNewPw)}><Ionicons name={showNewPw ? "eye-off-outline" : "eye-outline"} size={18} color="#94a3b8" /></TouchableOpacity></View></View>
          <View style={s.formField}><Text style={s.formLabel}>Confirm New Password</Text><View style={s.pwInputWrap}><TextInput style={s.pwInput} value={confirmPassword} onChangeText={setConfirmPassword} placeholder="Confirm password" placeholderTextColor="#9ca3af" secureTextEntry={!showConfirmPw} /><TouchableOpacity onPress={() => setShowConfirmPw(!showConfirmPw)}><Ionicons name={showConfirmPw ? "eye-off-outline" : "eye-outline"} size={18} color="#94a3b8" /></TouchableOpacity></View></View>
          <View style={s.pwRules}>
            {[{ rule: "At least 8 characters", ok: newPassword.length >= 8 }, { rule: "One uppercase letter", ok: /[A-Z]/.test(newPassword) }, { rule: "One number", ok: /[0-9]/.test(newPassword) }, { rule: "Passwords match", ok: newPassword === confirmPassword && confirmPassword.length > 0 }].map(r => (
              <View key={r.rule} style={s.pwRuleRow}><Ionicons name={r.ok ? "checkmark-circle" : "ellipse-outline"} size={14} color={r.ok ? "#16a34a" : "#94a3b8"} /><Text style={[s.pwRuleText, { color: r.ok ? "#16a34a" : "#94a3b8" }]}>{r.rule}</Text></View>
            ))}
          </View>
  ...
  const BOTTOM_TABS: { key: TabType; iconName: keyof typeof Ionicons.glyphMap; iconActive: keyof typeof Ionicons.glyphMap; label: string; badge?: number }[] = [
        ...
        <ScrollView style={s.tabScroll} contentContainerStyle={[s.tabContainer, { paddingBottom: bottomNavHeight + 20 }]} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#16a34a"]} />}>
          <View style={s.topBar}>
            <View style={s.topBarLeft}>
              <View style={s.topBarLogoCircle}><Image source={require("../assets/images/logo.png")} style={s.topBarLogoImg} resizeMode="contain" /></View>
              <Text style={s.topBarTitle}>UniFiX</Text>
            </View>
            <View style={s.topBarRight}>
              <TouchableOpacity style={s.bellBtn}><Ionicons name="notifications-outline" size={18} color="#374151" /></TouchableOpacity>
              <TouchableOpacity onPress={() => setActiveTab("profile")}>
                {staffData?.photoUrl
                  ? <Image source={{ uri: staffData.photoUrl }} style={s.topBarAvatar} />
                  : <View style={s.topBarAvatarEmpty}><Text style={s.topBarAvatarText}>{staffData?.fullName?.[0]?.toUpperCase() || "S"}</Text></View>}
              </TouchableOpacity>
            </View>
          </View>
          <View style={s.welcomeBanner}>
            ...
            <View style={s.welcomeDateRow}><Ionicons name="calendar-outline" size={13} color="#64748b" /><Text style={s.welcomeDate}>{today}</Text></View>
          </View>
          <View style={s.statsGrid}>
            {STAT_ITEMS.map(stat => (
              <View key={stat.label} style={s.statCard}>
                <View style={[s.statIconWrap, { backgroundColor: stat.iconBg }]}><Ionicons name={stat.iconName} size={20} color={stat.iconColor} /></View>
          ...
          {allTasks.length === 0 ? (
            <View style={s.emptyState}>
              <View style={s.emptyIconWrap}><Ionicons name="checkmark-circle-outline" size={36} color="#16a34a" /></View>
      ...
      {activeTab === "lostfound" && (
        <View style={{ flex: 1 }}>
          <View style={s.lfPageHeader}>
            <TouchableOpacity onPress={() => setActiveTab("tasks")} style={s.lfPageBackBtn}><Ionicons name="arrow-back" size={18} color="#0f172a" /></TouchableOpacity>
  ...
  {lostFoundTab === "claims" && (claimItems.length === 0 ? (
    ...
    <View key={item.id} style={s.lfCard}>
      ...
      {activeTab === "history" && (
        <ScrollView style={s.tabScroll} contentContainerStyle={[s.tabContainer, { paddingBottom: bottomNavHeight + 20 }]} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#16a34a"]} />}>
          <View style={s.subHeader}><Text style={s.subHeaderTitle}>History</Text></View>
          <View style={s.historyStats}>
            <View style={s.historyItem}><Text style={[s.historyNum, { color: "#16a34a" }]}>{completed.length}</Text><Text style={s.historyLabel}>Completed</Text></View>
            <View style={s.historyDivider} />
            <View style={s.historyItem}><Text style={[s.historyNum, { color: "#dc2626" }]}>{rejected.length}</Text><Text style={s.historyLabel}>Rejected</Text></View>
          </View>
          {completed.length === 0 && rejected.length === 0 && (
            <View style={s.emptyState}><View style={s.emptyIconWrap}><Ionicons name="time-outline" size={36} color="#94a3b8" /></View><Text style={s.emptyText}>No history yet</Text></View>
          )}
          {completed.length > 0 && (
            <>
              <View style={s.historySection}><Ionicons name="checkmark-circle" size={14} color="#16a34a" style={{ marginRight: 6 }} /><Text style={s.historySectionTitle}>Completed</Text></View>
              {completed.map(c => renderTaskCard(c))}
            </>
          )}
          {rejected.length > 0 && (
            <>
              <View style={s.historySection}><Ionicons name="close-circle" size={14} color="#dc2626" style={{ marginRight: 6 }} /><Text style={[s.historySectionTitle, { color: "#dc2626" }]}>Rejected by You</Text></View>
      ...
      <View style={[s.bottomBar, { paddingBottom: insets.bottom + 10, height: bottomNavHeight }]}>
        {BOTTOM_TABS.map(tab => {
          const isActive = activeTab === tab.key;
          return (
            <TouchableOpacity key={tab.key} style={s.bottomTab} onPress={() => { setActiveTab(tab.key); setProfileScreen("main"); }}>
              <View style={{ position: "relative" }}>
                <Ionicons name={isActive ? tab.iconActive : tab.iconName} size={22} color={isActive ? "#16a34a" : "#94a3b8"} />
                {tab.badge != null && tab.badge > 0 && (
                  <View style={s.badgeDot}><Text style={s.badgeDotText}>{tab.badge}</Text></View>
                )}
              </View>
              <Text style={[s.bottomLabel, isActive && s.bottomLabelActive]}>{tab.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
      ...
      <Modal visible={detailVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setDetailVisible(false)}>
        <View style={s.modalRoot}>
          <View style={s.modalTopBar}>
            <TouchableOpacity onPress={() => setDetailVisible(false)} style={s.modalBackBtn}><Ionicons name="arrow-back" size={18} color="#0f172a" /></TouchableOpacity>
          ...
          {selectedComplaint && (
            <ScrollView contentContainerStyle={s.modalContent}>
              {selectedComplaint.photoUrl ? (
                <TouchableOpacity onPress={() => setImageViewerUri(selectedComplaint.photoUrl!)} activeOpacity={0.9}>
                  <Image source={{ uri: selectedComplaint.photoUrl }} style={s.modalPhoto} resizeMode="cover" />
                </TouchableOpacity>
              ) : null}
              <Text style={s.modalIssueTitle}>{selectedComplaint.subIssue || selectedComplaint.customIssue || "Issue"}</Text>
              <View style={[s.statusPill, { backgroundColor: (STATUS_CONFIG[selectedComplaint.status] || STATUS_CONFIG.pending).bg, alignSelf: "flex-start", marginBottom: 16 }]}>
                <Text style={[s.statusPillText, { color: (STATUS_CONFIG[selectedComplaint.status] || STATUS_CONFIG.pending).color }]}>{(STATUS_CONFIG[selectedComplaint.status] || STATUS_CONFIG.pending).label}</Text>
              </View>
              <Text style={s.modalSectionLabel}>REPORTER</Text>
              <View style={s.reporterCard}>
                <View style={s.reporterAvatar}><Text style={s.reporterAvatarText}>{selectedComplaint.submittedByName?.[0]?.toUpperCase()}</Text></View>
                <View style={{ flex: 1 }}>
                  <Text style={s.reporterName}>{selectedComplaint.submittedByName}</Text>
                  {selectedComplaint.submittedByPhone ? <Text style={s.reporterPhone}>{selectedComplaint.submittedByPhone}</Text> : null}
                </View>
                <TouchableOpacity style={s.callBtn} onPress={() => handleCall(selectedComplaint.submittedByPhone)}>
                  <Ionicons name="call-outline" size={20} color="#16a34a" />
                </TouchableOpacity>
              </View>
              <Text style={s.modalSectionLabel}>LOCATION</Text>
              <View style={s.locationCard}>
                <View style={s.locationCardIcon}><Ionicons name="location-outline" size={20} color="#16a34a" /></View>
                <View><Text style={s.locationCardMain}>{selectedComplaint.building || "Campus"}</Text><Text style={s.locationCardSub}>{selectedComplaint.roomDetail || "—"}</Text></View>
              </View>
              {selectedComplaint.description ? (
                <><Text style={s.modalSectionLabel}>DESCRIPTION</Text><View style={s.descCard}><Text style={s.descText}>{selectedComplaint.description}</Text></View></>
              ) : null}
              <View style={s.modalActions}>
                ...
                {selectedComplaint.status === "assigned" && (
                  <TouchableOpacity style={s.inProgressActionBtn} onPress={() => handleUpdateStatus(selectedComplaint.id, "in_progress")} disabled={!!actionLoading}>
                    {actionLoading === selectedComplaint.id ? <ActivityIndicator color="#7c3aed" /> : <Text style={s.inProgressText}>Mark as In Progress</Text>}
                  </TouchableOpacity>
                )}
                {selectedComplaint.status === "in_progress" && (
                  <TouchableOpacity style={s.acceptActionBtn} onPress={() => handleUpdateStatus(selectedComplaint.id, "completed")} disabled={!!actionLoading}>
                    {actionLoading === selectedComplaint.id ? <ActivityIndicator color="#fff" /> : <Text style={s.acceptActionBtnText}>Mark as Completed</Text>}
                  </TouchableOpacity>
                )}
                {(selectedComplaint.status === "completed" || selectedComplaint.status === "rejected") && (
                  <View style={s.resolvedBanner}><Text style={s.resolvedBannerText}>{selectedComplaint.status === "completed" ? "Task completed" : "Task rejected"}</Text></View>
                )}
              </View>
            </ScrollView>
          )}
      ...
      <Modal visible={rejectVisible} animationType="fade" transparent onRequestClose={() => setRejectVisible(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={s.sheetOverlay}>
          <View style={[s.sheet, { paddingBottom: insets.bottom + 24 }]}>
            ...
            <Text style={s.sheetSub}>Please provide a reason for rejection</Text>
            <TextInput style={s.sheetInput} placeholder="Enter reason here..." placeholderTextColor="#9ca3af" multiline numberOfLines={4} value={rejectReason} onChangeText={setRejectReason} textAlignVertical="top" />
            <View style={s.sheetBtnRow}>
              <TouchableOpacity style={s.sheetCancelBtn} onPress={() => { setRejectVisible(false); setDetailVisible(true); }}><Text style={s.sheetCancelText}>Cancel</Text></TouchableOpacity>
              <TouchableOpacity style={[s.sheetRejectBtn, !!actionLoading && { opacity: 0.55 }]} onPress={handleReject} disabled={!!actionLoading}>
                {actionLoading ? <ActivityIndicator color="#fff" /> : <Text style={s.sheetRejectText}>Confirm Reject</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </KeyboardAvoidingView>
      ...
      <Modal visible={!!handoverItem} animationType="slide" transparent onRequestClose={() => setHandoverItem(null)}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={s.sheetOverlay}>
          <View style={[s.sheet, { paddingBottom: insets.bottom + 24 }]}>
            ...
            <TextInput style={[s.sheetInput, { minHeight: 50, textAlignVertical: "auto" }]} placeholder="e.g. Soham Patil" placeholderTextColor="#9ca3af" value={handedToName} onChangeText={t => { setHandedToName(t); setHandoverError(""); }} autoCapitalize="words" />
            {handoverError ? <Text style={{ color: "#dc2626", fontSize: 13, marginBottom: 8, fontWeight: "500" }}>{handoverError}</Text> : null}
            <View style={s.sheetBtnRow}>
              <TouchableOpacity style={s.sheetCancelBtn} onPress={() => setHandoverItem(null)} disabled={handoverLoading}><Text style={s.sheetCancelText}>Cancel</Text></TouchableOpacity>
      ...
      {toast && (
        <Animated.View
          style={[s.toast, toast.type === "success" && s.toastSuccess, toast.type === "error" && s.toastError, toast.type === "info" && s.toastInfo, { transform: [{ translateY: toastAnim.interpolate({ inputRange: [0, 1], outputRange: [-80, 0] }) }], opacity: toastAnim }]}
          pointerEvents="none"
        >
          <Ionicons name={toast.type === "success" ? "checkmark-circle" : toast.type === "error" ? "close-circle" : "information-circle"} size={18} color="#fff" />
...
const s = StyleSheet.create({
  ...
  topBar: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingTop: 56, paddingBottom: 16 },
  topBarLeft: { flexDirection: "row", alignItems: "center", gap: 10 },
  topBarLogoCircle: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#f0fdf4", borderWidth: 1.5, borderColor: "#bbf7d0", alignItems: "center", justifyContent: "center", overflow: "hidden" },
  ...
  topBarAvatar: { width: 36, height: 36, borderRadius: 18, borderWidth: 2, borderColor: "#16a34a" },
  topBarAvatarEmpty: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#16a34a", alignItems: "center", justifyContent: "center" },
  topBarAvatarText: { color: "#fff", fontSize: 14, fontWeight: "800" },
  welcomeBanner: { backgroundColor: "#f0fdf4", borderRadius: 16, padding: 20, marginBottom: 16, borderWidth: 1, borderColor: "#bbf7d0" },
  welcomeLabel: { fontSize: 11, fontWeight: "700", color: "#16a34a", letterSpacing: 1, marginBottom: 6 },
  welcomeName: { fontSize: 22, fontWeight: "800", color: "#0f172a", marginBottom: 10, letterSpacing: -0.3 },
  welcomeDateRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  welcomeDate: { fontSize: 13, color: "#64748b", fontWeight: "500" },
  statsGrid: { flexDirection: "row", flexWrap: "wrap", gap: 12, marginBottom: 20 },
  statCard: { width: "47%", backgroundColor: "#ffffff", borderRadius: 14, padding: 14, borderWidth: 1.5, borderColor: "#f1f5f9", shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 },
  ...
  statValue: { fontSize: 26, fontWeight: "800", color: "#0f172a", marginBottom: 2 },
  ...
  taskCard: { backgroundColor: "#ffffff", borderRadius: 16, overflow: "hidden", marginBottom: 14, borderWidth: 1.5, borderColor: "#f1f5f9", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  taskPhoto: { width: "100%", height: 180 },
  urgentBadge: { position: "absolute", top: 12, right: 12, borderRadius: 6, paddingHorizontal: 10, paddingVertical: 4 },
  urgentBadgeText: { fontSize: 10, fontWeight: "800", color: "#fff", letterSpacing: 0.5 },
  statusBadgeTop: { alignSelf: "flex-start", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4, marginBottom: 8 },
  statusBadgeTopText: { fontSize: 11, fontWeight: "700" },
  taskBody: { padding: 14 },
  taskTitle: { fontSize: 16, fontWeight: "700", color: "#0f172a", marginBottom: 8, lineHeight: 22 },
  taskCatBadge: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: "#f0fdf4", borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4, alignSelf: "flex-start", marginBottom: 10 },
  taskCatText: { fontSize: 10, fontWeight: "700", color: "#16a34a", letterSpacing: 0.3 },
  taskLocationRow: { flexDirection: "row", alignItems: "center", gap: 5, marginBottom: 10 },
  taskLocationText: { fontSize: 13, color: "#64748b", flex: 1 },
  taskReporterRow: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 },
  taskReporterAvatar: { width: 24, height: 24, borderRadius: 12, backgroundColor: "#f0fdf4", alignItems: "center", justifyContent: "center" },
  taskReporterAvatarText: { fontSize: 11, fontWeight: "700", color: "#16a34a" },
  taskReporterName: { fontSize: 12, color: "#64748b" },
  taskRatingRow: { flexDirection: "row", alignItems: "center", gap: 4, marginBottom: 12, backgroundColor: "#fffbeb", borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
  taskRatingText: { fontSize: 11, color: "#92400e", fontWeight: "600", marginLeft: 4 },
  ...
  progressBtn: { flex: 1, backgroundColor: "#7c3aed", borderRadius: 10, paddingVertical: 12, alignItems: "center" },
  detailBtn: { flex: 1, backgroundColor: "#ffffff", borderRadius: 10, paddingVertical: 12, alignItems: "center", borderWidth: 1.5, borderColor: "#e2e8f0" },
  ...
  subHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingTop: 56, paddingBottom: 16, paddingHorizontal: 4, borderBottomWidth: 1, borderBottomColor: "#f1f5f9", marginBottom: 16 },
  subHeaderTitle: { fontSize: 18, fontWeight: "800", color: "#0f172a" },
  lfPageHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingTop: 56, paddingHorizontal: 20, paddingBottom: 14, backgroundColor: "#ffffff", borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
  ...
  historyStats: { backgroundColor: "#ffffff", borderRadius: 14, padding: 20, flexDirection: "row", alignItems: "center", marginBottom: 20, borderWidth: 1.5, borderColor: "#f1f5f9" },
  historyItem: { flex: 1, alignItems: "center" },
  historyNum: { fontSize: 28, fontWeight: "800" },
  historyLabel: { fontSize: 13, color: "#64748b", fontWeight: "600", marginTop: 4 },
  historyDivider: { width: 1, height: 40, backgroundColor: "#f1f5f9" },
  historySection: { flexDirection: "row", alignItems: "center", marginBottom: 10, marginTop: 4 },
  historySectionTitle: { fontSize: 14, fontWeight: "700", color: "#16a34a", letterSpacing: 0.2 },
  rejectedReasonBox: { backgroundColor: "#fef2f2", borderRadius: 8, padding: 10, marginTop: 8, borderWidth: 1, borderColor: "#fecaca" },
  rejectedReasonLabel: { fontSize: 11, fontWeight: "700", color: "#dc2626", marginBottom: 3 },
  rejectedReasonText: { fontSize: 13, color: "#7f1d1d", lineHeight: 18 },
  profileCard: { backgroundColor: "#ffffff", borderRadius: 16, padding: 24, alignItems: "center", marginBottom: 14, borderWidth: 1.5, borderColor: "#f1f5f9" },
  profileAvatarBtn: { position: "relative", marginBottom: 12 },
  profileAvatar: { width: 86, height: 86, borderRadius: 43, backgroundColor: "#f0fdf4", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#16a34a" },
  profileAvatarImg: { width: 86, height: 86, borderRadius: 43, borderWidth: 2, borderColor: "#16a34a" },
  profileAvatarText: { fontSize: 34, fontWeight: "800", color: "#16a34a" },
  profileCameraBtn: { position: "absolute", bottom: 0, right: 0, width: 28, height: 28, borderRadius: 8, backgroundColor: "#16a34a", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#fff" },
  profileName: { fontSize: 20, fontWeight: "800", color: "#0f172a", marginBottom: 6 },
  profileRoleBadge: { backgroundColor: "#f0fdf4", borderRadius: 20, paddingHorizontal: 14, paddingVertical: 5, borderWidth: 1.5, borderColor: "#bbf7d0", marginBottom: 4 },
  profileRoleBadgeText: { fontSize: 11, fontWeight: "700", color: "#16a34a", letterSpacing: 0.5 },
  profileHint: { fontSize: 12, color: "#94a3b8", marginBottom: 10 },
  idBadge: { backgroundColor: "#f8fafc", borderRadius: 20, paddingHorizontal: 14, paddingVertical: 5, borderWidth: 1.5, borderColor: "#e2e8f0", marginBottom: 14 },
  idBadgeText: { fontSize: 12, fontWeight: "600", color: "#374151" },
  profilePerfBox: { width: "100%", backgroundColor: "#f8fafc", borderRadius: 12, padding: 14, borderWidth: 1, borderColor: "#e2e8f0" },
  profilePerfLabel: { fontSize: 10, fontWeight: "700", color: "#94a3b8", letterSpacing: 0.5, marginBottom: 4 },
  profilePerfScore: { fontSize: 20, fontWeight: "800", color: "#0f172a", marginBottom: 2 },
  profilePerfSub: { fontSize: 14, fontWeight: "700", color: "#0f172a", marginBottom: 2 },
  ...
  settingsList: { backgroundColor: "#ffffff", borderRadius: 14, overflow: "hidden", borderWidth: 1.5, borderColor: "#f1f5f9", marginBottom: 20 },
  settingRow: { flexDirection: "row", alignItems: "center", padding: 16, borderBottomWidth: 1, borderBottomColor: "#f8fafc", gap: 14 },
  ...
  subPageHeader: { flexDirection: "row", alignItems: "center", gap: 14, marginBottom: 20, paddingTop: 56 },
  ...
  formCard: { backgroundColor: "#ffffff", borderRadius: 14, padding: 18, marginBottom: 16, borderWidth: 1.5, borderColor: "#f1f5f9" },
  ...
  bottomBar: { flexDirection: "row", backgroundColor: "#ffffff", borderTopWidth: 1, borderTopColor: "#f1f5f9", paddingTop: 10, position: "absolute", bottom: 0, left: 0, right: 0, shadowColor: "#000", shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.04, shadowRadius: 8, elevation: 8 },
  ...
  badgeDot: { position: "absolute", top: -3, right: -6, backgroundColor: "#ef4444", borderRadius: 8, minWidth: 16, height: 16, alignItems: "center", justifyContent: "center", paddingHorizontal: 3 },
  badgeDotText: { color: "#fff", fontSize: 9, fontWeight: "800" },
  modalRoot: { flex: 1, backgroundColor: "#f8fafc" },
  modalTopBar: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingTop: 56, paddingBottom: 14, paddingHorizontal: 20, backgroundColor: "#ffffff", borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
  modalBackBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: "#f1f5f9", alignItems: "center", justifyContent: "center" },
  ...
  modalSectionLabel: { fontSize: 11, fontWeight: "700", color: "#94a3b8", letterSpacing: 0.8, marginTop: 4 },
  reporterCard: { backgroundColor: "#ffffff", borderRadius: 14, padding: 14, flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1.5, borderColor: "#f1f5f9" },
  reporterAvatar: { width: 46, height: 46, borderRadius: 12, backgroundColor: "#f0fdf4", alignItems: "center", justifyContent: "center" },
  ...
  callBtn: { width: 42, height: 42, borderRadius: 10, backgroundColor: "#f0fdf4", alignItems: "center", justifyContent: "center", borderWidth: 1.5, borderColor: "#bbf7d0" },
  locationCard: { backgroundColor: "#ffffff", borderRadius: 14, padding: 14, flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1.5, borderColor: "#f1f5f9" },
  locationCardIcon: { width: 42, height: 42, borderRadius: 10, backgroundColor: "#f0fdf4", alignItems: "center", justifyContent: "center" },
  locationCardMain: { fontSize: 14, fontWeight: "700", color: "#0f172a" },
  locationCardSub: { fontSize: 13, color: "#94a3b8", marginTop: 2 },
  descCard: { backgroundColor: "#ffffff", borderRadius: 14, padding: 14, borderWidth: 1.5, borderColor: "#f1f5f9" },
  ...
  inProgressActionBtn: { flex: 1, backgroundColor: "#ede9fe", borderRadius: 12, paddingVertical: 14, alignItems: "center" },
  inProgressText: { color: "#7c3aed", fontSize: 14, fontWeight: "700" },
  completeActionBtn: { flex: 1, backgroundColor: "#f8fafc", borderRadius: 12, paddingVertical: 14, alignItems: "center", borderWidth: 1.5, borderColor: "#e2e8f0" },
  completeText: { color: "#374151", fontSize: 14, fontWeight: "700" },
  rejectActionBtn: { backgroundColor: "#fff", borderRadius: 12, paddingVertical: 14, alignItems: "center", borderWidth: 1.5, borderColor: "#fecaca" },
  rejectActionText: { color: "#dc2626", fontSize: 14, fontWeight: "700" },
  resolvedBanner: { backgroundColor: "#f0fdf4", borderRadius: 12, paddingVertical: 14, alignItems: "center", borderWidth: 1, borderColor: "#bbf7d0" },
  ...
  toast: { position: "absolute", top: 60, left: 20, right: 20, flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 16, paddingVertical: 14, borderRadius: 14, zIndex: 9999, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.12, shadowRadius: 12, elevation: 10 },
