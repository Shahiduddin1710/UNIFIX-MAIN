...
const CATEGORIES = ["Electronics", "Clothing", "Stationery", "ID Card", "Keys", "Bag", "Water Bottle", "Earphones", "Books", "Others"];
...
export default function PostFoundItemScreen() {
...
const pickPhoto = async () => {
    Alert.alert("Add Photo", "Choose an option", [
      {
        ...
        onPress: async () => {
          try {
            ...
            setPhoto({ uri: asset.uri, name: asset.uri.split("/").pop() || `lostfound_${Date.now()}.jpg` });
  ...
  const handleSubmit = async () => {
    ...
    if (!collectLocation.trim()) return setError("Please mention where to collect the item.");
    setSubmitting(true);
    try {
      ...
      await lostFoundAPI.postItem({
        ...
        roomLabel: resolvedRoom.label,
        collectLocation: collectLocation.trim(),
        photoUrl,
      });
      router.replace("/lost-and-found" as any);
    } catch (err: any) {
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={0}>
      ...
      <View style={s.root}>
        <View style={s.header}>
          ...
          <Text style={s.headerTitle}>Post Found Item</Text>
          <View style={{ width: 36 }} />
        </View>
        <ScrollView contentContainerStyle={s.container} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" showsVerticalScrollIndicator={false}>
          <TouchableOpacity style={s.photoCard} onPress={pickPhoto} activeOpacity={0.85}>
            {photo ? (
              ...
              <View style={s.photoPlaceholder}>
                ...
                <Text style={s.photoPlaceholderTitle}>Upload Image</Text>
                <Text style={s.photoPlaceholderSub}>Add photos of the item to help identify it</Text>
          ...
          <View style={s.section}>
            ...
            <TextInput style={s.input} placeholder="e.g. Black Leather Wallet" placeholderTextColor="#9ca3af" value={itemName} onChangeText={setItemName} />
            ...
            <TextInput style={[s.input, s.textarea]} placeholder="Describe color, brand, unique marks..." placeholderTextColor="#9ca3af" value={description} onChangeText={setDescription} multiline textAlignVertical="top" />
          </View>

          <View style={s.section}>
            <Text style={s.sectionTitle}>Location Details</Text>
            <Text style={s.label}>Where Found (Room Number)</Text>
            ...
            <Text style={s.label}>Where to Collect</Text>
            <TextInput style={[s.input, s.textarea]} placeholder="e.g., Room 214 after 2PM, or Staff Room 501" placeholderTextColor="#9ca3af" value={collectLocation} onChangeText={setCollectLocation} multiline textAlignVertical="top" />
          </View>
          ...
          <Text style={s.disclaimer}>By clicking Submit Claim, you agree to our Terms of Service regarding false claims.</Text>
        </ScrollView>
      </View>
...
const s = StyleSheet.create({
  ...
  disclaimer: { textAlign: "center", fontSize: 11, color: "#94a3b8", lineHeight: 18, paddingHorizontal: 10 },
});